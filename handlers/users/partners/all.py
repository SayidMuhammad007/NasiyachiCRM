...
async def bot_echo(message: types.Message, state: FSMContext):
    ...
    dataa = await getAll(table="🏢 Hamkor-do'konlar")
    data = dataa
...
@dp.callback_query_handler(lambda c: c.data in ['prevPartner', 'nextPartner'])
async def process_callback(callback_query: types.CallbackQuery, state: FSMContext):
    global current_page
    global data  # Use the global data variable
    if callback_query.data == 'prevPartner':
        current_page = max(0, current_page - 1)
    elif callback_query.data == 'nextPartner':
