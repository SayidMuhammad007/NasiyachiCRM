...
from keyboards.default.mainBtn import confirmBtn0
from loader import dp, bot
from sheet import add_row, getData1


@dp.callback_query_handler(lambda callback_query: callback_query.data.startswith('cancelCancel_'))
async def handle_product_deletion(callback_query: types.CallbackQuery):
    selected = callback_query.data.split('_')[1]
    print(f"ale{selected}")
    ...
    add = await add_row(rows=[["AU", int(selected) + 3, "🟡 Qabul qilindi!"], ["AH", int(selected) + 3, worker[0][19]]],
                        table="🏢 Hamkor-do'konlar")
